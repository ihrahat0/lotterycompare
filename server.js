...
app.post('/api/images/upload', verifyToken, upload.single('image'), async (req, res) => {
    try {
        ...
        const filePath = `admin-uploads/${imageKey}-${Date.now()}${path.extname(req.file.originalname)}`;
        ...
        if (storageError) {
            ...
            return res.status(500).json({ error: storageError.message, details: storageError });
        }
